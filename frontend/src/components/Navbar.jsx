...
import { Bell, Search, User, LogOut } from 'lucide-react'; // Import LogOut icon
...
const Navbar = () => {
  ...
  const [showUserInfoCard, setShowUserInfoCard] = useState(false); // New state for info card visibility
  ...
  const handleBrowseBooks = (e) => {
    e.preventDefault();
    // Use navigate directly for consistency if browse-books is also a route
    navigate(`/browse-books`); // Removed searchTerm if "browse all"
  };
  ...
  // Toggle user info card visibility
  ...
  const handleLogout = () => {
    ...
    setShowUserInfoCard(false); // Close the info card after logout
  ...
  return (
    <div>
      {!isAuthenticated ? (
        // Logged out navbar
        <div className="w-full mx-auto bg-white rounded-lg p-4 flex items-center justify-between text-nowrap shadow-md">
          ...
          <div className="flex items-center gap-6 ml-4">
            ...
            <div className="flex items-center gap-2">
              <button onClick={handleLoginButton} className={`px-4 py-2 text-gray-800 font-medium hover:text-[${buttonGreen}]`}>Log In</button>
              <button onClick={handleSignUpButton} className={`px-4 py-2 bg-green-500 text-white font-medium rounded-lg hover:bg-[${buttonGreen}]`}>Sign Up</button>
            </div>
          </div>
        </div>
      ) : (
        // Logged in navbar
        <div className="w-full mx-auto bg-white rounded-lg p-4 flex flex-col md:flex-row md:items-center md:justify-between text-nowrap shadow-md">
          {/* Left Section */}
          ...
          <div className="flex justify-end md:order-3 items-center gap-4">
            <button
              className={`px-4 py-2 bg-green-500 text-white font-medium rounded-lg hover:bg-[${buttonGreen}]`}
            ...
            {/* User Icon and Info Card */}
            <div className="relative"> {/* Parent for positioning dropdown */}
              <button
              ...
              {showUserInfoCard && user && ( // Conditionally render if showUserInfoCard is true and user data exists
                <div className="absolute right-0 mt-2 w-64 bg-white border border-gray-200 rounded-lg shadow-lg z-10 p-4 text-gray-800">
                  <p className="font-bold text-base">Assalamu Alaikum,</p>
                  <p className="font-bold text-lg mb-2"> {user.name || 'User'}!</p>
                  <p className="text-sm">Email: {user.email}</p>
                  {user.contact && <p className="text-sm">Contact: {user.contact}</p>}
                  {/* You can add more user info here if available, e.g., role, hall */}
                  
                  <button
                    onClick={handleLogout}
                    className={`mt-4 w-full flex items-center justify-center gap-2 px-4 py-2 bg-red-500 text-white font-medium rounded-lg hover:bg-red-600 focus:outline-none focus:ring-2 focus:ring-red-500`}
                  >
