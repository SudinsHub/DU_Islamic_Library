...
const InsertBookForm = () => {
    ...
    const [formData, setFormData] = useState({
        ...
        hall_id: '', // New state for selected hall ID
    });
    ...
    useEffect(() => {
        const fetchHalls = async () => {
            ...
            try {
                ...
                setHallsError('Network error or server issue while fetching halls. Please try again.');
            } finally {
    ...
    const handleSubmit = async (e) => {
        ...
        if (!formData.title || !formData.author || !formData.publisher || !formData.category || !formData.hall_id) {
            setFormError('Please fill in all required fields (Title, Author, Publisher, Category, and Hall).');
        ...
        formDataToSend.append('hall_id', formData.hall_id); // Append selected hall ID
        ...
        try {
            ...
            if (response.success) {
                ...
                // Reset form fields after successful submission, keep copies_to_add and selectedHallId
                setFormData(prev => ({
                    ...prev,
                    title: '',
                    author: '',
    ...
    return (
        <div className="p-4 sm:p-6 bg-white rounded-lg shadow-sm">
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <div className="mb-6">
                    <label htmlFor="hall-select" className="block text-sm font-medium text-gray-700 mb-1">
                    ...
                    >
                        <option value="">{loadingHalls ? 'Loading Halls...' : 'Select a Hall'}</option>
                        {halls.map((hall) => (
                    ...
                    {hallsError && <p className="text-red-500 text-sm mt-2">{hallsError}</p>}
                </div>
                ...
                <Button
                    ...
                    disabled={formLoading || loadingHalls} // Disable button if halls are still loading
                >
